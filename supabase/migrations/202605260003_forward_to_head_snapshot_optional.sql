...
declare
  ...
  has_snapshot_evidence boolean := false;
...
begin
  ...
  has_snapshot_evidence := normalized_storage_path is not null or comments_json_has_evidence;
  should_create_version := has_snapshot_evidence;

  if input_decision = 'request_revision' and not has_snapshot_evidence then
    raise exception 'Snapshot koreksi atau comments_json wajib tersedia';
  end if;

  if input_decision = 'request_revision'
    and normalized_source_type = 'manual_snapshot_upload'
    and normalized_storage_path is null
  then
    raise exception 'Snapshot koreksi wajib diupload';
  end if;

  if input_snapshot_job_id is not null and not should_create_version then
    raise exception 'Job snapshot koreksi memerlukan evidence snapshot';
  end if;
  ...
  next_revision_round := draft_letter.revision_round;
  corrected_version_id := latest_version_id;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'created_correction_version', should_create_version,
      'revision_round', next_revision_round,
      'source_type', case when should_create_version then normalized_source_type else null end,
      ...
      'exported_at', case when should_create_version then resolved_exported_at else null end
    )
  );
